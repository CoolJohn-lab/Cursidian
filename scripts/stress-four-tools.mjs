...
function record(kind, name, detail) {
  ...
  console.log(
    `${tag} ${name}${detail ? `: ${typeof detail === 'string' ? detail : JSON.stringify(detail).slice(0, 240)}` : ''}`,
  );
}
...
async function main() {
  ...
  // --- Prior bug regressions ---
  {
    ...
    if (data) {
      ...
      if (tokens.includes('home') && !tokens.includes('note')) {
        record('fail', 'regression: note->home typo', {
          contentTokens: tokens,
          correctedTokens: data.correctedTokens,
        });
  ...
  // --- Graph stress ---
  {
    ...
    record(graphFails ? 'warn' : 'ok', `graph: scanned ${paths.length} notes`, {
      graphFails,
      maxOut,
      maxBack,
    });
...

  {
    ...
    for (const p of [
      ...
      if (!r.isError && p.includes('..')) {
        ...
        if (
          data.note &&
          !data.note.replace(/\\/g, '/').startsWith('entities/') &&
          p.includes('entities/cursidian')
        ) {
...

  {
    ...
    const hash =
      d1?.contentHash ??
      parseResult(await callTool(server, 'note', { action: 'read', path: noteA })).contentHash;
  ...
  // --- Concurrent-ish: interleaved search while mutating ---
  {
    ...
    for (let i = 0; i < 20; i++) {
      ops.push(
        callTool(server, 'search', {
          action: 'content',
          query: 'Stress APPENDED_MARKER',
          limit: 5,
        }),
      );
      ops.push(
        callTool(server, 'search', { action: 'by_tags', tags: ['stress-probe'], limit: 20 }),
      );
  ...
  // --- Vault stress ---
  {
    ...
    const r2 = await callTool(server, 'vault', {
      action: 'list_folders',
...

  {
    const health = expectOk(
      'vault: health',
      await callTool(server, 'vault', { action: 'health', staleDays: 1 }),
    );
    if (health) {
      // our probe notes may appear as orphans / missing FM issues - that's ok, note them
      const probeOrphans = (health.orphans ?? []).filter(
        (p) => String(p).includes('_stress-') || String(p.path ?? '').includes('_stress-'),
      );
      if (probeOrphans.length) {
        record(
          'warn',
          'vault health lists stress probes as orphans (expected mid-run)',
          probeOrphans.length,
        );
      }
    }
  ...
  // --- Dispatch confusion / invalid combos ---
  {
    const combos = [
      ...
      [
        'note',
        { action: 'update', path: noteB, mode: 'patch', old_string: 'nope', new_string: 'x' },
      ],
      [
        'note',
        {
    ...
    for (const [tool, args] of combos) {
      try {
        ...
        if (!r.isError) {
          record(
            'fail',
            `invalid combo should error: ${tool} ${JSON.stringify(args).slice(0, 80)}`,
            'succeeded',
          );
        } else {
  ...
  // cleanup probe notes
  {
    const list = parseResult(
      await callTool(server, 'search', { action: 'list', folder: '_raw', recursive: true }),
    );
  ...
  {
    const health = expectOk(
      'vault: health after cleanup',
      await callTool(server, 'vault', { action: 'health' }),
    );
