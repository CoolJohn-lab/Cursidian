export async function runEdgeCaseSuite(ctx) {
  ...
  results.push(
    await runCase(
      'search token-AND excludes partial matches',
      async () => {
  ...
  results.push(
    await runCase(
      'search ranks title match above body-only match',
      async () => {
        const data = parseResult(
          await callTool(server, 'search', {
            action: 'content',
  ...
  results.push(
    await runCase(
      'search empty query rejected by schema',
      async () => {
  ...
  results.push(
    await runCase(
      'note read missing file',
      async () => {
        const result = await callTool(server, 'note', {
          action: 'read',
          path: '__missing-note-xyz__',
        });
  ...
  results.push(
    await runCase(
      'note read path traversal blocked',
      async () => {
        const result = await callTool(server, 'note', {
          action: 'read',
          path: '../../../etc/passwd',
  ...
  results.push(
    await runCase(
      'note update patch ambiguous old_string',
      async () => {
  ...
  results.push(
    await runCase(
      'note update replace size guard',
      async () => {
  ...
  results.push(
    await runCase(
      'note update replace allows large shrink with force',
      async () => {
  ...
  results.push(
    await runCase(
      'graph resolves path-style wikilink backlinks',
      async () => {
        const data = parseResult(
  ...
  results.push(
    await runCase(
      'note read resolves outgoing links on index',
      async () => {
