...
export async function runSmokeSuite(ctx) {
  ...
  try {
    ...
    results.push(
      await runCase('search recent', async () => {
        const data = parseResult(await callTool(server, 'search', { action: 'recent', limit: 5 }));
        if (!Array.isArray(data.notes)) throw new Error('missing notes');
    ...
    results.push(
      await runCase('note read revisionHash', async () => {
    ...
    results.push(
      await runCase('note update expectedRevision', async () => {
        const data = parseResult(
          await callTool(server, 'note', {
            action: 'update',
            path: smokePath,
    ...
    results.push(
      await runCase('revision conflict structured error', async () => {
    ...
    results.push(
      await runCase('invalid_args recovery', async () => {
        const err = parseError(
          await callTool(server, 'note', {
            action: 'read',
            path: smokePath,
    ...
    results.push(
      await runCase('vault health', async () => {
    ...
    results.push(
      await runCase('vault sync_index dryRun', async () => {
        const data = parseResult(
    ...
    results.push(
      await runCase('vault list_folders', async () => {
        parseResult(await callTool(server, 'vault', { action: 'list_folders', path: '' }));
      }, ctx),
    );

    results.push(
      await runCase('vault manifest upsert+read', async () => {
    ...
    results.push(
      await runCase('vault history', async () => {
        const data = parseResult(await callTool(server, 'vault', { action: 'history', limit: 20 }));
        if (!Array.isArray(data.operations)) throw new Error('missing operations');
        if (data.operations.length < 1) throw new Error('expected at least one journaled operation');
      }, ctx),
    );

    results.push(
      await runCase('vault undo reverse order', async () => {
