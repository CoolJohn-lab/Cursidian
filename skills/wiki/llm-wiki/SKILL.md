...
2. Reads use `search` (actions: `content`, `by_tags`, `list`, `recent`, `tags`), `note` (action: `read`), `graph`, `vault` (actions: `health`, `slop_check`, `history`, `manifest` with `manifestOperation: "read"`, `list_folders`). Writes use `note` (actions: `create`, `update`, `delete`, `rename`, `frontmatter`), `vault` (actions: `sync_index`, `deslop`, `create_folder`, `delete_folder`, `log`, `undo`, `manifest` mutations).
...
6. **Verification** - after multi-page write workflows: `note` `read` each changed page; `vault` `sync_index` with `dryRun: true` expecting `wouldWrite: false`; report residual drift instead of claiming success.
