...
6. **Verification** - after multi-page write workflows: `note` `read` each changed page; `vault` `sync_index` with `dryRun: true` expecting `wouldWrite: false` (flat: body matches generated catalog; hub: curated body unchanged - check health `indexMode`); report residual drift instead of claiming success. Also prefer `vault` `health` with `counts.indexDrift: 0` under hub mode.
