...
export function registerContext(server: McpServer, config: Config): void {
  server.registerTool(
    ...
    {
      description:
        'Assembles a token-budgeted, provenance-tagged context bundle - the CGE surface. Use for broad, uncertain, multi-page, connection, or task-briefing work; narrow or known-path reads may be cheaper through search, note, or graph. action=assemble (default): query + tokenBudget (server default 4000), optional intent (lookup|connection|onboarding|troubleshoot|ingest-prep, inferred from the query when omitted). Start around 300-500 for a summary skim, 600-1000 for routing, 1200-2500 for task context, and reserve 4000+ for deliberate body depth or synthesis. action=for_task: same assembly, phrased as a task description. action=expand: continue a prior bundle via its nextCursor with a fresh tokenBudget. action=feedback: record that a bundle was insufficient or off-target for a query (local vault log only, read-only vaults reject this). Bundles are read-only and compose search/graph internally; never exceed tokenBudget; items carry kind (summary|section|body|neighbor-note), score, reasons, provenance, staleDays, and citations as [[wikilinks]]. Response includes focus (1-3 primary paths) and guidance.nextStep (sufficient|expand|refine_query); use focus items as evidence and deepen only when guidance or exact-detail needs justify it.',
      inputSchema: {
        action: z
