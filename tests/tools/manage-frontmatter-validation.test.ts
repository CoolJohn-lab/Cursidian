...
describe('validateFrontmatterOperation', () => {
  const cases: Array<{
    ...
    { operation: 'set', data: { title: 'x' }, replaceAll: true, error: null },
    {
      ...
      data: { title: 'x' },
      error:
        'operation "set" replaces all frontmatter keys; pass replaceAll: true to confirm, or use merge instead',
    },
    {
      operation: 'set',
      error:
        'operation "set" requires frontmatter with at least one field (e.g. { title: "...", updated: "..." })',
    },
    {
      operation: 'set',
      data: {},
    ...
    { operation: 'merge', error: 'operation "merge" requires frontmatter with at least one field' },
    {
      operation: 'merge',
      data: {},
      error: 'operation "merge" requires frontmatter with at least one field',
    },
  ...
  it.each(cases)(
    '$operation returns expected validation result',
    ({ operation, data, keys, replaceAll, error }) => {
