...
describe('write -> search cache invalidation', () => {
  it('note create is immediately findable via search content', async () => {
    ...
    const data = parseResult(
      await callTool(ctx.server, 'search', { action: 'content', query: 'unique-create-token-xyzzy', limit: 10 }),
  ...
  it('note update patch is immediately findable via search content', async () => {
    ...
    await callTool(ctx.server, 'search', { action: 'content', query: 'before-patch-token', limit: 5 });

    const read = parseResult(
      await callTool(ctx.server, 'note', { action: 'read', path: 'cache-patch-visible.md' }),
    ...
    const data = parseResult(
      await callTool(ctx.server, 'search', { action: 'content', query: 'after-patch-token-unique', limit: 10 }),
    ) as { results: Array<{ path: string }> };
  ...
  it('note delete removes the note from search content immediately', async () => {
    ...
    await callTool(ctx.server, 'search', { action: 'content', query: 'unique-delete-token-plugh', limit: 5 });
    ...
    const data = parseResult(
      await callTool(ctx.server, 'search', { action: 'content', query: 'unique-delete-token-plugh', limit: 10 }),
    ) as { results: Array<{ path: string }> };
  ...
  it('note frontmatter tag changes are immediately searchable via tags filter', async () => {
    ...
    await callTool(ctx.server, 'search', {
      action: 'content',
